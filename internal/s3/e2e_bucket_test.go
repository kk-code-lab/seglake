...
func TestS3E2EDeleteBucketWithDeleteMarker(t *testing.T) {
	dir := t.TempDir()
	store, err := meta.Open(filepath.Join(dir, "meta.db"))
	if err != nil {
		t.Fatalf("meta.Open: %v", err)
	...
	if err := store.CreateBucketWithVersioning(context.Background(), "bucket", meta.BucketVersioningEnabled); err != nil {
