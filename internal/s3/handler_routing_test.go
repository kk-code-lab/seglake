...
func TestBucketListKindForRequest(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			method:        http.MethodGet,
			target:        "/demo?encryption",
			hostBucket:    "",
			hasBucketOnly: true,
			want:          bucketGetEncryption,
		},
		{
		...
		},
		{
			...
			hostBucket:    "",
			hasBucketOnly: true,
			want:          bucketDeleteEncryption,
		},
		{
			name:          "get-bucket-lifecycle",
			method:        http.MethodGet,
			target:        "/demo?lifecycle",
			hostBucket:    "",
			hasBucketOnly: true,
			want:          bucketGetLifecycle,
		},
		{
...
func TestIsListV1Request(t *testing.T) {
	cases := []struct {
		...
		{
			name:          "list-v1-ignored-on-encryption",
			method:        http.MethodGet,
			target:        "/demo?encryption",
			hasBucketOnly: true,
			want:          false,
		...
		{
			name:          "list-v1-ignored-on-lifecycle",
			method:        http.MethodGet,
			target:        "/demo?lifecycle",
			hasBucketOnly: true,
			want:          false,
