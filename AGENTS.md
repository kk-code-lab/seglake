...
- Use `make <target>` for builds/tests; targets include `build`, `run`, `test`, `test-coverage`, `test-race`, `test-e2e`, `test-all`, `fmt`, `lint`, `lint-astgrep`, `test-astgrep`, `check`.
...
- ast-grep: rules live in `rules/ast-grep/lint`, tests in `rules/ast-grep/tests` with snapshots; run `make lint-astgrep` and `make test-astgrep`.
