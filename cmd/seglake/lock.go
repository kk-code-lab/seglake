...
func isUnsafeLiveMode(mode string) bool {
	switch mode {
	case "rebuild-index", "gc-run", "gc-rewrite", "gc-rewrite-run", "mpu-gc-run", "sse-rewrap-run", "repl-pull", "repl-push", "repl-bootstrap", "db-integrity-check", "db-reindex":
		return true
	default:
		return false
	}
...
func allowsUnsafeInMaintenance(mode string) bool {
	switch mode {
	case "gc-run", "gc-rewrite", "gc-rewrite-run", "mpu-gc-run", "sse-rewrap-run":
		return true
	default:
		return false
	}
