...
func printModeHelp(mode string) {
	...
	case "keys":
		...
		fmt.Println("Flags: -keys-action list|create|allow-bucket|list-buckets|disable|delete, -key-access, -key-secret, -key-policy, -key-enabled, -key-inflight, -key-bucket, -json.")
	default:
