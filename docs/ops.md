...
- `/v1/meta/stats` includes `sse_diagnostics`, the same redacted metadata-only SSE summary used by support bundles. It also includes `lifecycle_diagnostics` with configured bucket/rule counts and per-bucket rule IDs/update times, but no lifecycle XML, filters, tag values, actions, or fingerprints. These fields are observability only; use `scrub -scrub-deep-encrypted` when you need decryptability and AEAD verification.
