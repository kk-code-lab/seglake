...
- SSE-S3 can be explicit per request or configured as a bucket default. Plaintext writes remain the default for buckets without default encryption unless `x-amz-server-side-encryption: AES256` is present, but native policies can require effective SSE-S3 for selected write paths.
...
| Object data / KEKs | Segments/manifests/config | Offline disclosure | Attacker copies data dir without KEKs | SSE-S3 envelope encryption for requests with `x-amz-server-side-encryption: AES256` or bucket default encryption; key-provider material is external config and is not stored in SQLite/manifests | code + ops | off unless requested per object or configured per bucket | `-sse-s3-enabled`, `-sse-s3-provider`, `-sse-s3-active-key`, `-sse-s3-kek`, or Vault Transit config; back up local KEKs/Vault separately | `internal/sse`, `internal/storage/engine`, `internal/s3` tests |
| Object data / KEKs | S3 write API | Accidental plaintext writes | Client forgets `--sse AES256` or bucket default is removed | Native policy condition `require_sse_s3: true` rejects plaintext PutObject, CopyObject destinations, and CreateMultipartUpload before bytes are committed; bucket default encryption also satisfies the requirement | code | off unless configured in identity or bucket policy | Use bucket/prefix resources to scope enforcement | `internal/s3/policy_test.go`, `internal/s3/policy_integration_test.go` |
