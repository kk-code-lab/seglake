...
| Bucket lifecycle config | S3 lifecycle API / replication | Metadata disclosure or unintended retention policy changes | Lifecycle rules reveal prefix/tag structure or are changed without authorization | `GetBucketLifecycle`/`PutBucketLifecycle`/`DeleteBucketLifecycle` are policy actions; config changes replicate as metadata-only oplog entries; lifecycle execution is not automatic in this phase | code | off unless configured | Treat lifecycle XML as operational metadata; use bucket policies to scope lifecycle reads/writes | `internal/s3/bucket_lifecycle_test.go`, `internal/meta/bucket_lifecycle_test.go` |
